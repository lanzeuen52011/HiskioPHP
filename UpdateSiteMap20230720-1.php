...
class UpdateSiteMap extends Command
{
    ...
    public function handle()
    {
        ...
        $fileName = 'Sitemap';
        $ext = '.xml';
        $siteMapIndexFileName = 'sitemapindex';
        $siteMapIndexFilePath = $publicPath . $siteMapIndexFileName . $ext;
        $filePath = $publicPath . $fileName . $ext;
        
        // A.舊檔案備份與改名區
           // 先將舊檔案改名，才不會使舊檔案被刪掉
           if(file_exists($siteMapIndexFilePath)){
               chmod($siteMapIndexFilePath, 0777); // 將檔案狀態改成可修改模式
               rename($siteMapIndexFilePath, $publicPath.$toDayDate . '.xml');
           }
        ...
        // C.端口處理區
            ...
            // 完成區
                ...
                $normalRoute = $this->normalRoute($path,$fileName);
                $sitemapindex->addSitemap(env('APP_URL').$normalRoute.$ext);
                ...
                dump("@相對端口\r\n");
                    ...
                    $companyPage = $this->companyPage($path,$fileName);
                    $sitemapindex->addSitemap(env('APP_URL').$companyPage.$ext);
                    // 2.各商標放入sitemap
                    $mark = $this->mark($path,$fileName);
                    $sitemapindex->addSitemap(env('APP_URL').$mark.$ext);
                    // 3.提供『專業服務』的公司
                    $classifiedCategory = $this->classifiedCategory($path,$fileName);
                    $sitemapindex->addSitemap(env('APP_URL').$classifiedCategory.$ext);
                    $sitemapindex->store('sitemapindex', $path.$siteMapIndexFileName);
                    // 終於找到如何將sitemap變成sitemapindex，請直接找到Class本身叫做Sitemap，以此專案為例路徑為vendor/laravelium/sitemap/src/Laravelium/Sitemap/Sitemap.php
                    // 並找尋store方法，將儲存副檔名處改成sitemapindex，就成功了
                    dump("檔案已儲存到".$path.$siteMapIndexFileName);
                dd("結束");
    ...
    public function normalRoute($path,$fileName){
        ...
        $executionTime = $endTime - $startTime;
        dump("總花費秒數：",$executionTime);
        return $path.$siteMapFolderNormalRoute.$siteMapFileName;
    }
    public function companyPage($path,$fileName){
        ...
        // 正式版
            // $all = PrimaryCompany::count();
            // PrimaryCompany::select('tax_number', 'name','updated_at)
            //                 ->withCount('sameNames') // @必要，假設五間公司相同名字，不帶這個會變成只給出一間的資訊，這裡會抓出tax_number來讓各間公司作出區分，並給出相對應的route
            //                 ->chunk($batchSize,function($companies)use($sitemap,$all,$batchSize){
            //                     foreach ($companies as $company) {
            //                         static $i = 0;
        ...
        // 本地測試版
            $desiredStatus = ['核准登記','核准設立'];
            $all = PrimaryCompany::whereIn('status', $desiredStatus)->whereNotNull('created_at')->count();
            // dd($all);
            $companies = PrimaryCompany::
                              whereIn('status', $desiredStatus) // 測試用才增加的，正式版沒有
                            ->whereNotNull('created_at')// 測試用才增加的，正式版沒有
                            ->select('tax_number', 'name','updated_at')
                            ->withCount('sameNames') // @必要，假設五間公司相同名字，不帶這個會變成只給出一間的資訊，這裡會抓出tax_number來讓各間公司作出區分，並給出相對應的route
                            ->cursor();
            foreach ($companies as $company) {
                static $i = 0;
                ...
                // 處理每個資料
                $sitemap->add(route('detail_by_zh', $company->getRouteParams()), $company->updated_at);
                $percentage = ($i / $all) * 100;
                if ($percentage % 10 === 0 || $percentage === 10) {
                    echo $percentage, '%', " ";
                }
            }
                            // @chunk 100筆，不要設定太高，以限制效能，先不要用cursor怕SQL會崩潰
        ...
        $executionTime = $endTime - $startTime;
        dump("總花費秒數：",$executionTime);
        return  $path.$siteMapFolderCompanyPage.$siteMapFileName;
    ...
    function mark($path,$fileName){
        ...
        $executionTime = $endTime - $startTime;
        dump("總花費秒數：",$executionTime);
        return $path.$siteMapFolderMark.$siteMapFileName;
    }
    public function classifiedCategory($path,$fileName){
        ...
        $executionTime = $endTime - $startTime;
        dump("總花費秒數：",$executionTime);
        return $path.$siteMapFolderClassified.$siteMapFileName;
    }
}